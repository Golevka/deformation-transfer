 ...
 * of size nz.  Row indexes of column j are held in i [p [j] ... p [j+1]-1] and
 * in the same locations in x.  There may be no duplicate entries in a column.
 * Row indexes in each column may be sorted or unsorted (CHOLMOD keeps track).
 ...
 * cholmod_sort			sort row indexes in each column of sparse matrix
...
typedef struct cholmod_sparse_struct
{
    ...
    void *i ;		/* i [0..nzmax-1], the row indexes */
 ...
 * In all cases, the row indexes in the columns of L are kept sorted. */
...
typedef struct cholmod_factor_struct
{
    ...
    void *nz ;		/* nz [0..ncol-1], the # of nonzeros in each column.
			 * i [p [j] ... p [j]+nz[j]-1] contains the row indexes,
...
typedef struct cholmod_triplet_struct
{
    ...
    void *i ;		/* i [0..nzmax-1], the row indexes */
    void *j ;		/* j [0..nzmax-1], the column indexes */
    void *x ;		/* size nzmax or 2*nzmax, if present */
    void *z ;		/* size nzmax, if present */
	...
	* new row and column indexes, via an inverse permutation.  Suppose
